...
const modeClasses = computed(() =>
  colorMode.value === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-white text-slate-900'
)
const headerClasses = computed(() => (colorMode.value === 'dark' ? 'bg-slate-900' : 'bg-white'))
const sidebarClasses = computed(() =>
  colorMode.value === 'dark' ? 'bg-slate-950 border-slate-800' : 'bg-gray-100 border-gray-300'
)
...
watch(
  route,
  () => {
    const current = allTreeItems.find(i => i.children?.some(c => c.to === route.path))
    if (current) {
